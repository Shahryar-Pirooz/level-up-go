...
func matchSales(budget float64, items []SaleItem) []SaleItem {
	bigSale := make([]SaleItem , 0)
	for _, item := range items {
		if item.ReducedPrice <= budget {
			item.SalePercentage = ((item.OriginalPrice - item.ReducedPrice) / item.OriginalPrice ) * 100
			bigSale = append(bigSale, item)
			budget -= item.ReducedPrice
		}
	}
	sort.Slice(bigSale , func(i, j int) bool {
		return bigSale[i].SalePercentage > bigSale[j].SalePercentage
	})
	return bigSale
}
